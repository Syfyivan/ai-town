...
const GARDEN_TILE_X = 43;
const GARDEN_TILE_Y = 30;
const GARDEN_WIDTH_TILES = 7;
const GARDEN_HEIGHT_TILES = 5;
...
function buildGardenSprite(tileDim: number) {
  const width = GARDEN_WIDTH_TILES * tileDim;
  const height = GARDEN_HEIGHT_TILES * tileDim;
  ...
  drawPixelRect(graphics, 0x8f563b, tileDim * 0.45, tileDim, width - tileDim * 0.9, height - tileDim * 1.45);
  ...
  for (let row = 0; row < 2; row += 1) {
    for (let col = 0; col < 2; col += 1) {
      ...
      drawPixelRect(graphics, 0x6abe30, x + tileDim * 0.2, y + tileDim * 0.16, tileDim * 0.35, tileDim * 0.32);
      drawPixelRect(graphics, 0x99e550, x + tileDim * 0.85, y + tileDim * 0.18, tileDim * 0.42, tileDim * 0.34);
    }
  }

  drawPixelRect(graphics, 0xfec742, tileDim * 1.45, tileDim * 0.25, tileDim * 4.1, tileDim * 0.48);
  drawPixelRect(graphics, 0x181425, tileDim * 0.15, tileDim * 4.12, width - tileDim * 0.3, tileDim * 0.25);
...
export const GardenHotspot = PixiComponent('GardenHotspot', {
  create: (props: GardenHotspotProps) => {
    ...
    container.x = GARDEN_TILE_X * props.tileDim;
    container.y = GARDEN_TILE_Y * props.tileDim;
  ...
  applyProps: (
    ...
    if (oldProps.tileDim !== newProps.tileDim) {
      instance.x = GARDEN_TILE_X * newProps.tileDim;
      instance.y = GARDEN_TILE_Y * newProps.tileDim;
    }
